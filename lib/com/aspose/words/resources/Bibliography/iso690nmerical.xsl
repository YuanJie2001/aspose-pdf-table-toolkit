      ...
      <xsl:when test="b:StyleNameLocalized">
        <xsl:choose>
          ...
          <xsl:when test="b:StyleNameLocalized/b:Lcid='1066'">
            <xsl:text>ISO 690 - Tham chiếu Số</xsl:text>
          </xsl:when>
          ...
          <xsl:when test="b:StyleNameLocalized/b:Lcid='9242'">
            <xsl:text>ISO 690 - Numerička referenca</xsl:text>
          </xsl:when>
          <xsl:when test="b:StyleNameLocalized/b:Lcid='10266'">
            <xsl:text>ISO 690 - Нумеричка референца</xsl:text>
          </xsl:when>
          <xsl:otherwise>
            <xsl:text>ISO 690 - Numerical Reference</xsl:text>
          </xsl:otherwise>
		...
		<xsl:if test="string-length(b:Corporate)=0">
			<xsl:for-each select="b:NameList/b:Person">
				...
				<xsl:if test="($cIsEtAl = '0' and $cMaxAllow >= position()) or ($cIsEtAl = '1' and not (position()>$cEtAlPosition)) ">
					<xsl:choose>
						<xsl:when test="local-name(../..)='com.vector.Main'">
