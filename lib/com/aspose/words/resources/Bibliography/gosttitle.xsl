...
<xsl:variable name="constGost_IntervieweeHeader">
      ...
      <xsl:when test="b:StyleNameLocalized">
        <xsl:choose>
          ...
          <xsl:when test="b:StyleNameLocalized/b:Lcid='1051'">
            <xsl:text>Gost – Zoradenie nadpisov</xsl:text>
          </xsl:when>
          <xsl:when test="b:StyleNameLocalized/b:Lcid='1060'">
            <xsl:text>GOST – razvrščanje naslovov</xsl:text>
          </xsl:when>
          <xsl:when test="b:StyleNameLocalized/b:Lcid='9242'">
            <xsl:text>GOST - Sortiranje naslova</xsl:text>
          </xsl:when>
          <xsl:when test="b:StyleNameLocalized/b:Lcid='10266'">
            <xsl:text>GOST - Сортирање наслова</xsl:text>
          </xsl:when>
          <xsl:otherwise>
            <xsl:text>GOST - Title Sort</xsl:text>
          </xsl:otherwise>
	...
	<xsl:template name="sortedList">
		...
		<xsl:apply-templates select="msxsl:node-set($sourceRoot)/*">
			...
			<xsl:sort select="b:Author/b:com.vector.Main/b:NameList/b:Person[1]/b:Last" />
			
			<xsl:sort select="b:Author/b:com.vector.Main/b:NameList/b:Person[1]/b:First" />
			
			<xsl:sort select="b:Author/b:com.vector.Main/b:NameList/b:Person[1]/b:Middle"/>
	...
	<xsl:template name="formatAuthor">
		<xsl:if test="string-length(b:Author/b:com.vector.Main/b:Corporate)=0">
		...
		<xsl:if test="string-length(b:Author/b:com.vector.Main/b:Corporate)>0">
			  <xsl:call-template name="templ_str_AuthorShortUnCap"/><xsl:call-template name="templ_prop_Space"/>
			  <xsl:value-of select="b:Author/b:com.vector.Main/b:Corporate"/>
		</xsl:if>
	</xsl:template>
		...
		<xsl:element name="{$Type}">
          ...
          <b:Author>
						...
						<xsl:for-each select="./b:Author/*">
							...
							<xsl:if test="name()!='b:com.vector.Main'">
								<xsl:element name="{name()}" namespace="{namespace-uri()}">
	...
	<xsl:template name ="formatHyphenSection">
		...
		<xsl:if test="not (string-length($FirstElement)=0 and string-length($SecondElement)=0)">
			...
			<xsl:if test="string-length($FirstElement)>0 and string-length($SecondElement)>0">
	...
	<xsl:template name ="formatPrimaryResponsibilities">
		...
		<xsl:param name ="MainSectionsCount">0</xsl:param>
		  ...
		  <xsl:if test ="$NonEmptyPrimSectionsCount >2">
		  ...
		  <xsl:if test ="not ($FirstElement ='') and $NonEmptyPrimSectionsCount > 1">
		  ...
		  <xsl:if test ="($FirstElement ='' and $NonEmptyPrimSectionsCount >1) or ($NonEmptyPrimSectionsCount > 2)">
    ...
    <xsl:param name ="FourthElement"></xsl:param>
      ...
      <xsl:if test ="string-length($CollectionName)>0 or string-length($FirstElement)>0 or string-length($SecondElement)>0 or string-length($ThirdElement)>0">
        <xsl:choose>
