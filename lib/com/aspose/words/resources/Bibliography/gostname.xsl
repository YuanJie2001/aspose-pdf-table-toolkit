      ...
      <xsl:when test="b:StyleNameLocalized">
        <xsl:choose>
          ...
          <xsl:when test="b:StyleNameLocalized/b:Lcid='9242'">
            <xsl:text>GOST - Sortiranje imena</xsl:text>
          </xsl:when>
          <xsl:when test="b:StyleNameLocalized/b:Lcid='10266'">
            <xsl:text>GOST - Сортирање имена</xsl:text>
          </xsl:when>
          <xsl:otherwise>
            <xsl:text>GOST - Name Sort</xsl:text>
          </xsl:otherwise>
						...
						<xsl:for-each select="msxsl:node-set($SortedList)/b:Bibliography/b:Source">
                            ...
                            <xsl:call-template name="templ_prop_GroupSeparator"/>													
								...
								<xsl:variable name ="MainSectionsCount">
									<xsl:choose>
										<xsl:when test ="count(b:Author/b:com.vector.Main/b:NameList/b:Person) >0 or string-length(b:Author/b:com.vector.Main/b:Corporate)>0">
...
<xsl:variable name="constGost_PersonAndOther">
	...
	<xsl:template name="formatMainAuthor">
		<xsl:if test="string-length(b:Author/b:com.vector.Main/b:Corporate)=0">
			<xsl:if test ="count(b:Author/b:com.vector.Main/b:NameList/b:Person) >0">
